...
{'timezones': ['Africa/Sao_Tome'], 'code': 'ST', 'continent': 'Africa', 'name': 'S\xc3\xa3o Tom\xc3\xa9 and Pr\xc3\xadncipe', 'capital': 'Sao Tome'},
...
def sortsecond():
    ...
    piz = {}
    tiz = []
    f = open("C:\\Users\saulw\Documents\Python\Projects\World capitals learner\scores.py", "r")
    filez = f.read()
    f.close()
    j=0
    l=0
    ...
    sorted_dicz = sorted(piz.items(), key=operator.itemgetter(1))

    gon = int(len(sorted_dicz))
    gon = gon*2

    for i in range(gon):
        tiz.append(sorted_dicz[l][k])
        if k==1:
            l+=1
            k=0
        else:
...
t=0
noms = []
continents = ["Africa","Europe","Oceania","North America", "South America", "Asia"]
name = input("What is your name?")
name = name.replace(" ","_")
while g<1:
    continent = input("Would you like to play with all countries (type all), a specific continent (type specific) or a random continent (type random)?").lower()
    if continent == "all" or continent == "a":
        k= -1000
        r=10
        choice = "all"
    elif continent == "specific" or continent == "s":
        for i in continents:
            print (i)
        while c<1:
            choice = input ("Input a continent from the above").title()
            if choice in continents:
                c+=1
            else:
                print ("Invalid input, please type a continent")
    elif continent == "random" or continent == "r":
         choice = random.choice(continents)
         print ("Your chosen continent is",choice)
    else:
        g=-1
...
for i in range (10):
    nom = random.randint(0,192)
    noms.append(nom)
    while k>0:
        if countries[nom]['continent']==choice:
            k=-1
        else:
            pass            
            nom = random.randint(0,192)
    print ("The capital of", countries[nom]["name"],"is:")
    guess = input ("Guess").lower()
    if guess == countries[nom]["capital"].lower():
        print ("Correct!")
        points +=1
    else:
        print("Sorry, the answer was", countries[nom]["capital"])
    if r<3:
        k=1
    countries.remove(countries[nom])
    countries.insert(len(countries),{'timezones': ['null'], 'code': 'null', 'continent': 'null', 'name': 'null', 'capital': 'null'})
    
print ("Well done, you got", points, "points\n")
now = time.time()
choice = choice.replace(" ","_")

time = (round(now-was,2))
print ("It took you", time, "seconds, that's", round(time/10,2), "seconds per city",)

f = open("C:\\Users\saulw\Documents\Python\Projects\World capitals learner\scores.py", "r")
filez = f.read()

f = open("C:\\Users\saulw\Documents\Python\Projects\World capitals learner\scores.py", "w")
filly = filez + "\n" + name+" "+str(points) + " (" + choice + ") " + str(time)+" seconds"
file = f.write(filly)
...
ro = 1
while ro>0:
    see = input("Would you like to see the top of the leaderboard, sorted by score or time? Type score, time or no").lower()
    if see == "s" or see == "score":
        sortscore()
        ro+=1
    elif see == "t" or see == "time":
        sortsecond()
        ro += 1
        ...
        print ("This is invalid. Please input score, time or no")
